...
JavaScript定义了一些被当做“假值”的特定值，因为当它们被强制转换为boolean类型时，它们的值会变为`false`——这些值包括`0`和`""`等。任何其他的值则自动表示“真值”——当被强制转换为boolean类型时，它们的值为`true`，如99.99和"free"等。详细参加第二章的“真值&假值”一节。
